...
export class PedidoService {
    async createPedido(dto: CreatePedidoDTO) {
        ...
        dto.itens.forEach(i => { if (!i.produtoId || i.quantidade < 1 || i.precoUnitario < 0) throw new Error('Item inválido'); });
        const total = this.__calcularTotal(dto.itens);
        const created = await (PedidoModel as any).create({ ...dto, total, status: StatusPedido.CRIADO });
        return created;
    }
    async getPedidos() {
        return await (PedidoModel as any).findAll?.() ?? await (PedidoModel as any).find?.();
    }
    ...
    async updatePedido(id: string, patch: UpdatePedidoDTO) {
        ...
        if (patch.status) {
            this.__validarTransicaoStatus(atual.status as StatusPedido, patch.status as StatusPedido);
    ...
    __validarTransicaoStatus(atual: StatusPedido, novo: StatusPedido) {
        const mapa: Record<StatusPedido, Set<StatusPedido>> = {
            [StatusPedido.CRIADO]: new Set([StatusPedido.PAGO, StatusPedido.CANCELADO]),
            [StatusPedido.PAGO]: new Set([StatusPedido.ENVIADO, StatusPedido.CANCELADO]),
